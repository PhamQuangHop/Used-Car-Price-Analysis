...
class CategoricalUnivariateAnalysis(UnivariateAnalysisStrategy):
    def analyze(self, df: pd.DataFrame, feature: str, top_k: int = 10):
        """
        Vẽ phân phối biến phân loại. Nếu số danh mục > top_k, gộp các danh mục ít gặp vào 'Other'
        và chỉ hiển thị Top-k theo tần suất.
        ...
        # Nếu quá nhiều danh mục, chỉ giữ lại top_k danh mục phổ biến nhất
        if len(counts) > top_k:
            top_cats = counts.nlargest(top_k).index
            s_plot = s[s.isin(top_cats)]
        else:
            s_plot = s

        # Tính lại thứ tự theo tần suất để cột sắp xếp giảm dần
        order = s_plot.value_counts().sort_values(ascending=False).index
